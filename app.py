...
# CSS: Kotak transparan untuk semua konten agar teks terbaca jelas
st.markdown("""
    <style>
    .stApp {
...
with tab2:
    ...
    if not data.empty and len(data) > 14:
        ...
        # Ambil nilai terakhir dan pastikan angka
        ...
        st.error("Data saham tidak tersedia untuk ticker ini.")

# Trading Advisor
if st.button("Generate Trading Summary"):
    if api_key and st.session_state.sentimen_data:
        client = OpenAI(api_key=api_key)
        res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": f"Analisis {ticker}. RSI: {st.session_state.rsi_val}. Sentimen: {st.session_state.sentimen_data}. Rekomendasi?"}])
