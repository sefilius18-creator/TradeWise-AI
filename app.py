...
# 1. Konfigurasi Halaman & CSS agar Tulisan Kontras
...
st.markdown("""
    ...
    /* Membuat kotak transparan agar tulisan selalu terbaca */
    .main > div { 
...
with tab2:
    try:
        # Mengunduh data dengan pengecekan aman
        data = yf.download(ticker, period="3mo", progress=False)
        
        # FIX: Pengecekan data.empty yang benar
        if data is not None and not data.empty and 'Close' in data.columns and len(data) >= 14:
            rsi = calculate_rsi(data)
            val = rsi.iloc[-1]
            
            # FIX: Pastikan val bukan NaN sebelum ditampilkan
            if pd.notna(val):
        ...
        st.error(f"Gagal memuat data teknikal: {e}")
